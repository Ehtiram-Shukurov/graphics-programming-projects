...
string textureName = "assets/test.ppm";

int texMode = 0;   // 0 = first texture, 1 = second texture

bool animate = false; // for animation
bool brightenTexture = false;
float brightenFactor = 1.5f;

const float INITIAL_HALF_WIDTH = 0.3f;
const float EDGE_DETECTION_BUFFER = 0.02f;
const float CORNER_CLICK_RADIUS = 0.02f;
...
void mouseClicked(float m_x, float m_y){
    if (animate) return;  // disable manual transforms while animating
   ...
   Dir2D v = Point2D(m_x, m_y) - rect_pos;
   float c = cos(-rect_angle);
   float s = sin(-rect_angle);
   float lx = c * v.x - s * v.y;
   float ly = s * v.x + c * v.y;

   float half = INITIAL_HALF_WIDTH * rect_scale;
   float edge = EDGE_DETECTION_BUFFER * rect_scale;
   float corner = CORNER_CLICK_RADIUS * rect_scale;
   bool inside = fabs(lx) < (half - edge) && fabs(ly) < (half - edge);

   if (inside) {
       do_translate = true;
       return;
   }
   auto distance = [](Point2D a, Point2D b) {
	   return (a - b).magnitude();
       };

   Point2D clickPt(m_x, m_y);
   if (distance(clickPt, p1) < corner ||
       distance(clickPt, p2) < corner ||
       distance(clickPt, p3) < corner ||
       distance(clickPt, p4) < corner) {
       do_scale = true;
       printf("Corner clicked -> scaling mode\n");
       return;
   }

   if ((fabs(fabs(lx) - half) <= edge && fabs(ly) <= half + edge) ||
       (fabs(fabs(ly) - half) <= edge && fabs(lx) <= half + edge)) {
       do_translate = false;
       do_scale = false;
       do_rotate = true;
       printf("Edge clicked -> rotation mode\n");
       return;
   }
}
...
void mouseDragged(float m_x, float m_y){
   ...
   if (do_scale){
           Dir2D current_vec = cur_mouse - rect_pos;
           Dir2D clicked_vec = clicked_mouse - rect_pos;

           float curDist = current_vec.magnitude();
           float clickDist = clicked_vec.magnitude();

           if (clickDist > 0) {
               float ratio = curDist / clickDist;
               rect_scale = clicked_size * ratio;
           }
   }

   if (do_rotate){
      float dxCur = cur_mouse.x - rect_pos.x;
      float dyCur = cur_mouse.y - rect_pos.y;

      float dxClick = clicked_mouse.x - rect_pos.x;
      float dyClick = clicked_mouse.y - rect_pos.y;

      float curAngle = atan2(dyCur, dxCur);
      float clickAngle = atan2(dyClick, dxClick);

      float angleDiff = clickAngle - curAngle;

      rect_angle = clicked_angle + angleDiff;

      printf("Rotating: rect_angle = %f\n", rect_angle);
   }
   applyTransform();
}
      ...
      while (SDL_PollEvent(&event)){ // Process events (keyboard, mouse, window, etc) that have happened since the last time we checked for events
         switch (event.type) {
            ...
            case SDL_EVENT_KEY_UP:
               ...
               if (event.key.key == SDLK_B) {   // press B to brighten
                   brightenTexture = !brightenTexture;
                   ...
                   if (brightenTexture) {
                       for (int i = 0; i < img_w * img_h * 4; i += 4) {
                           img_data[i] = min(255, int(img_data[i] * brightenFactor));
                           img_data[i + 1] = min(255, int(img_data[i + 1] * brightenFactor));
                           img_data[i + 2] = min(255, int(img_data[i + 2] * brightenFactor));
                       }
                   }
                   ...
                   delete[] img_data;
                   printf("Brightness toggled: %s\n", brightenTexture ? "ON" : "OFF");
               }

               if (event.key.key == SDLK_T) {
                   texMode = 1 - texMode; // toggle between 0 and 1
                   textureName = (texMode == 0) ? "assets/test.ppm" : "assets/brick.ppm";

                   int img_w, img_h;
                   unsigned char* new_data = loadImage(img_w, img_h);
                   glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, img_w, img_h, 0,
                       GL_RGBA, GL_UNSIGNED_BYTE, new_data);
                   glGenerateMipmap(GL_TEXTURE_2D);

                   delete[] new_data;
                   printf("Texture toggled: %s\n", textureName.c_str());
               }
