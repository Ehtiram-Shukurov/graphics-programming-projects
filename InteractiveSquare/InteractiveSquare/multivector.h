...
struct MultiVector{
  ...
  MultiVector(HomogeneousPoint2D p): s(0), x(0), y(0), w(0), yw(p.x), wx(p.y), xy(p.w), wxy(0) {}
  ...
  operator std::string() const {
    char result[200];
    snprintf(result, sizeof(result), "%.2f + %.2fx + %.2fy + %.2fw + %.2fyw + %.2fwx + %.2fxy + %.2fwxy", s, x, y, w, yw, wx, xy, wxy);
    return result;
  }
